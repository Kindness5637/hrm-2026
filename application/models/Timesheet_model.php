<?php
	...
	public function get_company_leaves($company_id) {
		...
		$sql = 'SELECT * FROM xin_leave_applications WHERE company_id = ? ORDER BY leave_id DESC';
		$binds = array($company_id);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	...
	public function get_employee_leaves($id) {
		...
		$sql = 'SELECT * FROM xin_leave_applications WHERE employee_id = ? ORDER BY leave_id DESC' ;
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	...
	function get_all_employees_active() {
    $query = $this->db->query('SELECT * FROM xin_employees WHERE is_active = 1 and user_role_id!=1');
    return $query;
}
}
?>