...
class Xin_model extends CI_Model {
	...
	public function select_module_class($mClass,$mMethod) {
		...
		if($mClass=='dashboard') {
			...
			$arr['hr_create_quotes_active'] = 'active';
			$arr['project_open'] = 'active';
			return $arr;
	...
	public function get_notify_user_tickets($employee_id) {
	  $query = $this->db->query("SELECT * from xin_support_tickets where is_notify = '1' and employee_id = '$employee_id' order by ticket_id desc");
  	  return $query->result();
	}
