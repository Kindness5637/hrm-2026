...
class Settings extends MY_Controller {
	...
	public function logo_info() {
		...
		$profile = dirname(dirname(dirname(__FILE__))).'/uploads/logo/';
	...
	public function logo_favicon() {
		...
		if(in_array($ext3,$allowed3)){
			...
			$profile3 = dirname(dirname(dirname(__FILE__))).'/uploads/logo/favicon/';
	...
	public function email_info() {
		...
		if(!empty($cdata['smtp_password']) && !hrm_is_encrypted($cdata['smtp_password'])){
			$cdata['smtp_password'] = hrm_encrypt($cdata['smtp_password']);
		}
	...
	public function notification_center()
	{
		...
		// Directory overrides
		$overrides = $this->db->select('*')->from('xin_role_email_overrides')->order_by('role_type', 'ASC')->get()->result();

		// Department heads (resolved from xin_departments)
		$dept_heads = $this->db->select('d.department_id, d.department_name, d.employee_id, e.first_name, e.last_name, e.email')
			->from('xin_departments d')
			->join('xin_employees e', 'e.user_id = d.employee_id', 'left')
			->order_by('d.department_name', 'ASC')
			->get()->result();

		// Routing rules
		$rules = $this->db->select('*')->from('xin_notification_rules')->order_by('module', 'ASC')->order_by('event', 'ASC')->get()->result();

		// Outbox log (last 20)
		$outbox = array();
		if ($this->db->table_exists('xin_notification_outbox')) {
			$outbox = $this->db->select('*')->from('xin_notification_outbox')->order_by('outbox_id', 'DESC')->limit(20)->get()->result();
		}

		$data = array(
			'title'            => 'Notification Center | ' . $this->Xin_model->site_title(),
			'company_info_id'  => $company_info[0]->company_info_id,
			'email_type'       => $email_config[0]->email_type,
			'smtp_host'        => $email_config[0]->smtp_host,
			'smtp_username'    => $email_config[0]->smtp_username,
			...
			'smtp_port'        => $email_config[0]->smtp_port,
			'smtp_secure'      => $email_config[0]->smtp_secure,
			'cc_emails'        => isset($email_config[0]->notification_cc_emails) ? $email_config[0]->notification_cc_emails : '',
			'overrides'        => $overrides,
			'dept_heads'       => $dept_heads,
			'rules'            => $rules,
			'outbox'           => $outbox,
			'breadcrumbs'      => 'Notification Center',
			'path_url'         => 'notification_settings',
		);

		$role_resources_ids = $this->Xin_model->user_role_resource();
		if (in_array('60', $role_resources_ids)) {
			if (!empty($session)) {
				$data['subview'] = $this->load->view("admin/settings/notification_center", $data, TRUE);
	...
	// Save SMTP + CC config (Tab 1)
	public function notification_smtp_save()
	{
		if ($this->input->post('type') == 'notification_smtp') {
			$Return = array('result' => '', 'error' => '', 'csrf_hash' => '');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			...
			$cdata = array(
				...
				'smtp_port'               => $this->input->post('smtp_port'),
				'smtp_secure'             => $this->input->post('smtp_secure'),
				'notification_cc_emails'  => $this->input->post('notification_cc_emails'),
			);
			$this->Xin_model->update_email_config_record($cdata, 1);

			$Return['result'] = 'SMTP configuration saved.';
			$this->output($Return);
			exit;
		}
	}

	// Send test email (Tab 1)
	public function notification_test_send()
	{
		$Return = array('result' => '', 'error' => '', 'csrf_hash' => '');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();

		$to = $this->input->post('test_email');
	...
	// Save recipient overrides (Tab 2)
	public function notification_recipients_save()
	{
		if ($this->input->post('type') == 'notification_recipients') {
			$Return = array('result' => '', 'error' => '', 'csrf_hash' => '');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();

			$role_types = $this->input->post('role_type');
			$scope_ids  = $this->input->post('scope_id');
			$emails     = $this->input->post('override_email');

			if (is_array($role_types)) {
				foreach ($role_types as $i => $rt) {
					$eid = isset($emails[$i]) ? trim($emails[$i]) : '';
					$sid = isset($scope_ids[$i]) ? (int)$scope_ids[$i] : 0;

					if (!empty($eid) && !filter_var($eid, FILTER_VALIDATE_EMAIL)) {
						continue; // skip invalid
					}

					$this->db->where('role_type', $rt)->where('scope_id', $sid);
					$existing = $this->db->count_all_results('xin_role_email_overrides');

					if ($existing > 0) {
						$this->db->where('role_type', $rt)->where('scope_id', $sid);
						$this->db->update('xin_role_email_overrides', array(
							'override_email' => $eid,
							'updated_at'     => date('Y-m-d H:i:s'),
						));
					} else {
						$this->db->insert('xin_role_email_overrides', array(
							'role_type'      => $rt,
							'scope_id'       => $sid,
							'override_email' => $eid,
			...
			$Return['result'] = 'Recipient overrides saved.';
			$this->output($Return);
			exit;
		}
	}

	// Save routing rules (Tab 3)
	public function notification_routing_save()
	{
		if ($this->input->post('type') == 'notification_routing') {
			$Return = array('result' => '', 'error' => '', 'csrf_hash' => '');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();

			$modules = $this->input->post('module');
			$events  = $this->input->post('event');
			$roles   = $this->input->post('notify_roles');
			$enabled = $this->input->post('enabled');

			if (is_array($modules)) {
				foreach ($modules as $i => $mod) {
					$evt = isset($events[$i]) ? $events[$i] : '';
					$rle = isset($roles[$i]) ? $roles[$i] : '';
					$ena = (isset($enabled[$i]) && $enabled[$i] == '1') ? 1 : 0;

					$this->db->where('module', $mod)->where('event', $evt);
					$existing = $this->db->count_all_results('xin_notification_rules');

					if ($existing > 0) {
						$this->db->where('module', $mod)->where('event', $evt);
						$this->db->update('xin_notification_rules', array(
							'notify_roles' => $rle,
							'enabled'      => $ena,
							'updated_at'   => date('Y-m-d H:i:s'),
						));
					} else {
						$this->db->insert('xin_notification_rules', array(
							'module'       => $mod,
							'event'        => $evt,
							'notify_roles' => $rle,
							'enabled'      => $ena,
			...
			$Return['result'] = 'Routing rules saved.';
			$this->output($Return);
			exit;
		}
	}

	// Preview recipients for a module+event (Tab 3)
	public function notification_preview()
	{
		$Return = array('result' => '', 'error' => '', 'data' => array(), 'csrf_hash' => '');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		...
		if (empty($module) || empty($event)) {
			$Return['error'] = 'Module and event required.';
			$this->output($Return);
			exit;
		}

		// Build a minimal payload (no specific employee — just resolve role-level emails)
		$payload = array('employee_id' => 0, 'company_id' => 0, 'department_id' => 0);
		$recipients = hrm_resolve_recipients($module, $event, $payload);

		$Return['data'] = $recipients;
		$Return['result'] = count($recipients) . ' recipient(s) configured for ' . $module . ' → ' . $event;
		$this->output($Return);
		exit;
	}
}
