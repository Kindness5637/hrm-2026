...
public function birthDayStatus() {
...
hrsale_mail('hrmsystem@stalis.co.ke',
	...
	// leave > timesheet
	public function reconcile_leave() {
		...
		$data['breadcrumbs'] = $this->lang->line('left_reconcile_leave');
		$data['path_url'] = 'reconcile_leave';
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('46',$role_resources_ids)) {
			if(!empty($session)){
				$data['subview'] = $this->load->view("admin/timesheet/reconcile_leave", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
	...
	public function task_list() {
		...
		foreach($task->result() as $r) {
			...
			$data[] = array(
				$combhr,
				$r->task_name.'<br>'.$this->lang->line('xin_project').': <a href="'.site_url().'admin/project/detail/'.$r->project_id.'">'.$prj_name.'</a>',
				$tdate,
				$status,
				$ol,
				$f_name,
...
public function get_employees_leave() {
	...
	$remaining_leave = $this->Timesheet_model->count_total_days_leaves($leave_type_id,$employee_id);
...
// Validate and add info in database
public function add_reconcile_leave() {
	...
	if($this->input->post('add_type')=='leave') {
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				$user_department = $this->Xin_model->get_user_department($user_info[0]->department_id);
				...
				$email_data = array(
					...
					'reason'       => $this->input->post('reason'),
					'approver'     => $dept_user_info[0]->first_name,
				);
				$notify_payload = array(
					'employee_id'   => $this->input->post('employee_id'),
					'company_id'    => $this->input->post('company_id'),
					'department_id' => isset($user_info[0]->department_id) ? $user_info[0]->department_id : 0,
				);
				$notify_messages = array(
					'employee'        => array('subject' => 'Leave Reconciled',                 'body' => hrm_leave_email('reconciled', 'employee',        $email_data)),
					'department_head' => array('subject' => 'Leave Reconciled — '.$full_name,   'body' => hrm_leave_email('reconciled', 'department_head', $email_data)),
					'hr_manager'      => array('subject' => 'Leave Reconciled — '.$full_name,   'body' => hrm_leave_email('reconciled', 'hr_manager',      $email_data)),
				);
				$notify_results = hrm_notify('leave', 'reconciled', $notify_payload, $notify_messages);
				$notify_failed = array();
				foreach ($notify_results as $nr) { if (empty($nr['sent'])) $notify_failed[] = $nr['email']; }
				if (!empty($notify_failed)) {
					$Return['warning'] = 'Leave reconciled, but notification failed for: '.implode(', ', $notify_failed);
				}
...
$no_of_days = $workdays;
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				$notify_messages = array(
					'employee'        => array('subject' => 'Your Leave Request',               'body' => hrm_leave_email('submitted', 'employee',        $email_data)),
					'department_head' => array('subject' => 'Leave Request from '.$full_name,    'body' => hrm_leave_email('submitted', 'department_head', $email_data)),
					'hr_manager'      => array('subject' => 'Leave Request from '.$full_name,    'body' => hrm_leave_email('submitted', 'hr_manager',      $email_data)),
				);
				$notify_results = hrm_notify('leave', 'submitted', $notify_payload, $notify_messages);
				$notify_failed = array();
				foreach ($notify_results as $nr) { if (empty($nr['sent'])) $notify_failed[] = $nr['email']; }
				if (!empty($notify_failed)) {
	...
	if($this->input->post('update_type')=='leave') {
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				if($this->input->post('status') == 2){//approved
					...
					$notify_messages = array(
						'employee'        => array('subject' => 'Leave Approved',           'body' => hrm_leave_email('second_approved', 'employee',        $email_data)),
						'department_head' => array('subject' => 'Leave Approved',           'body' => hrm_leave_email('second_approved', 'department_head', $email_data)),
						'hr_manager'      => array('subject' => 'Leave Approved',           'body' => hrm_leave_email('second_approved', 'hr_manager',      $email_data)),
					);
					$notify_results = hrm_notify('leave', 'second_approved', $notify_payload, $notify_messages);
					$notify_failed = array();
					foreach ($notify_results as $nr) { if (empty($nr['sent'])) $notify_failed[] = $nr['email']; }
					if (!empty($notify_failed)) {
						$Return['warning'] = 'Leave approved, but notification failed for: '.implode(', ', $notify_failed);
					}
					...
					$notify_messages = array(
						'employee'        => array('subject' => 'Leave Rejected',           'body' => hrm_leave_email('rejected', 'employee',        $email_data)),
						'department_head' => array('subject' => 'Leave Rejected',           'body' => hrm_leave_email('rejected', 'department_head', $email_data)),
						'hr_manager'      => array('subject' => 'Leave Rejected',           'body' => hrm_leave_email('rejected', 'hr_manager',      $email_data)),
					);
					$notify_results = hrm_notify('leave', 'rejected', $notify_payload, $notify_messages);
					$notify_failed = array();
					foreach ($notify_results as $nr) { if (empty($nr['sent'])) $notify_failed[] = $nr['email']; }
					if (!empty($notify_failed)) {
						$Return['warning'] = 'Leave rejected, but notification failed for: '.implode(', ', $notify_failed);
					}


					//send to hr

				} else if($this->input->post('status') == 4){ //first approval
					...
					$notify_messages = array(
						'employee'        => array('subject' => 'Leave Request — Sent to HR',  'body' => hrm_leave_email('first_approved', 'employee',        $email_data)),
						'department_head' => array('subject' => 'Leave Request — Sent to HR',  'body' => hrm_leave_email('first_approved', 'department_head', $email_data)),
						'hr_manager'      => array('subject' => 'Leave Request — Approval Needed', 'body' => hrm_leave_email('first_approved', 'hr_manager', $email_data)),
					);
					$notify_results = hrm_notify('leave', 'first_approved', $notify_payload, $notify_messages);
					$notify_failed = array();
					foreach ($notify_results as $nr) { if (empty($nr['sent'])) $notify_failed[] = $nr['email']; }
					if (!empty($notify_failed)) {
						$Return['warning'] = 'Leave first-approved, but notification failed for: '.implode(', ', $notify_failed);
					}
