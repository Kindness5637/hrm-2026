...
class Employees extends MY_Controller {
	...
	public function add_employee() {
		...
		if($employee_id === '') {
			$last_emp = $this->db->query("SELECT employee_id FROM xin_employees ORDER BY user_id DESC LIMIT 1")->row();
			if($last_emp && preg_match('/^EMP(\d+)$/', $last_emp->employee_id, $matches)) {
				$next_num = intval($matches[1]) + 1;
			} else {
				$next_num = 1;
			}
			$employee_id = 'EMP' . str_pad($next_num, 4, '0', STR_PAD_LEFT);
			// Ensure uniqueness
			while($this->Employees_model->check_employee_id($employee_id) > 0) {
				$next_num++;
				$employee_id = 'EMP' . str_pad($next_num, 4, '0', STR_PAD_LEFT);
			}
		}
