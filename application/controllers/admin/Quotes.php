...
class Quotes extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	//	$country = $this->Xin_model->read_country_info($supplier[0]->country_id);
		...
		$data = array(
			'title' => 'Edit Quote #'.$quote_info[0]->quote_number,
			...
			'xin_title' => $quote_info[0]->xin_title,
	...
	//	$country = $this->Xin_model->read_country_info($supplier[0]->country_id);
		...
		$data = array(
			'title' => 'View Quote #'.$quote_info[0]->quote_number,
			'breadcrumbs' => 'View Quote',
		...
		$data = array();
          ...
          foreach($quotes->result() as $r) {
			...
			if($r->status == 0){
				...
				$status = '<span class="label label-success">'.$this->lang->line('xin_q_project_created').'</span>';
			} else if($r->status == 2){
				$status = '<span class="label label-primary">'.$this->lang->line('xin_in_progress').'</span>';
			} else if($r->status == 3){
				$status = '<span class="label label-success">'.$this->lang->line('xin_q_project_completed').'</span>';
			} else if($r->status == 4){
				$status = '<span class="label label-primary">'.$this->lang->line('xin_quote_invoiced').'</span>';
			} else if($r->status == 5){
				$status = '<span class="label label-success">'.$this->lang->line('xin_payment_paid').'</span>';
			} else if($r->status == 6){
				$status = '<span class="label label-danger">'.$this->lang->line('xin_quote_deffered').'</span>';
			} else if($r->status == 7){
				$status = '<span class="label label-danger">'.$this->lang->line('xin_q_project_deffered').'</span>';
			} else {
				$status = '<span class="label label-warning">'.$this->lang->line('xin_quoted_title').'</span>';
			}
	...
	public function create_new_quote() {
		...
		} else if($this->input->post('xin_title')==='') {
		...
		} else if($this->input->post('project_manager')==='') {
			$Return['error'] = "The project manager field is required.";
		} else if($this->input->post('project_coordinator')==='') {
			$Return['error'] = "The project coordinator field is required.";
		}
		...
		$clientinfo = $this->Clients_model->read_client_info($this->input->post('client_id'));
		$data = array(
		'quote_number' => $this->input->post('quote_number'),
		'xin_title' => $this->input->post('xin_title'),
	 ...
	 public function convert_to_invoice() {
		...
		$id = $this->uri->segment(4);
		$pid = $this->Project_model->read_project_information($id);
		if(is_null($pid)){
			redirect('admin/project/');
		}
		$quote_info = $this->Quotes_model->read_quote_info($pid[0]->quote_id);
		if(is_null($quote_info)){
			redirect('admin/project/');
		}
		// get customer
		...
		if ($result == TRUE) {
			foreach($this->Quotes_model->get_quote_items($quote_info[0]->quote_id) as $_item):
				...
				'item_tax_type' => $_item->tax_type,
				'item_tax_rate' => $_item->tax_figure,
		...
		redirect('admin/project/detail/'.$id);
	 }
	...
	public function delete() {
		...
		if($this->input->post('is_ajax') == '2') {
			...
			if($this->input->post('quote_password')==='') {
				$Return['error'] = "The Quote password field is required.";
			} else if($this->input->post('quote_password')!='1234') {
				$Return['error'] = "The Quote password is incorrect.";
			}
			...
			if(isset($id)) {
				$result_item = $this->Quotes_model->delete_quotes_items($id);
				$result = $this->Quotes_model->delete_qproject_record($id);
				$result = $this->Quotes_model->delete_qinvoice_record($id);
